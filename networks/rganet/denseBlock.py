...
class smooth_layer(nn.Module):
    def __init__(self, inChannel=CONFIG["NUM_CLS"], outChannel=CONFIG["NUM_CLS"], stack=3):
        super(smooth_layer, self).__init__()
        self.block1 = bottleNeck(inChannel, outChannel, stack)
        self.block2 = bottleNeck(outChannel, outChannel, stack)
        self.block3 = bottleNeck(2 * outChannel, outChannel, stack)
        self.block4 = bottleNeck(3 * outChannel, outChannel, stack)
        self.block5 = bottleNeck(4 * outChannel, outChannel, stack)
        self.block6 = bottleNeck(5 * outChannel, outChannel, stack)
        self.block7 = bottleNeck(6 * outChannel, outChannel, stack)
        self.block8 = bottleNeck(7 * outChannel, outChannel, stack)
        self.block9 = bottleNeck(8 * outChannel, outChannel, stack)
        self.norm1 = nn.BatchNorm2d(outChannel)
        self.norm2 = nn.BatchNorm2d(CONFIG["NUM_CLS"])
        self.connect = nn.Conv2d(outChannel, CONFIG["NUM_CLS"], 1, 1)

    def forward(self, feat):
        feat1 = self.block1(feat)
        feat2 = self.block2(feat1)
        feat3 = torch.cat((feat1, feat2), dim=1)
        feat4 = self.block3(feat3)
        feat5 = torch.cat((feat3, feat4), dim=1)
        feat6 = self.block4(feat5)
        feat7 = torch.cat((feat5, feat6), dim=1)
        feat8 = self.block5(feat7)
        feat9 = torch.cat((feat7, feat8), dim=1)
        feat10 = self.block6(feat9)
        feat11 = torch.cat((feat9, feat10), dim=1)
        feat12 = self.block7(feat11)
        feat13 = torch.cat((feat11, feat12), dim=1)
        feat14 = self.block8(feat13)
        feat15 = torch.cat((feat13, feat14), dim=1)
        feat16 = self.block9(feat15)
        feat16 = ops.leaky_relu(self.norm1(feat16))
        feat16 = self.connect(feat16)
        feat16 = ops.leaky_relu(self.norm2(feat16))
        return feat16
